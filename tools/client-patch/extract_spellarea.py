#!/usr/bin/env python3
"""Extract SpellArea.dbc from the newest client MPQ that has it."""
import ctypes
import sys
...
inner = (sys.argv[1] if len(sys.argv) > 1 else r"DBFilesClient\SpellArea.dbc").encode("ascii")
for rel in CANDIDATES:
    p = CLIENT / rel
    h = ctypes.c_void_p()
    if not SFileOpenArchive(str(p), 0, 0, ctypes.byref(h)):
        print(f"open FAILED {p.name}")
        continue
    fh = ctypes.c_void_p()
    ...
    print(f"  has file in {rel}, size={size:#x} hi={hi.value:#x}")
    ...
    read = wintypes.DWORD()
    storm.SFileReadFile.argtypes = [ctypes.c_void_p, ctypes.c_void_p, wintypes.DWORD, ctypes.POINTER(wintypes.DWORD), ctypes.c_void_p]
    SFileReadFile(fh, buf, size, ctypes.byref(read), None)
    SFileCloseFile(fh)
    SFileCloseArchive(h)
    out = Path(__file__).resolve().parent / "cache" / "SpellArea.dbc.base"
