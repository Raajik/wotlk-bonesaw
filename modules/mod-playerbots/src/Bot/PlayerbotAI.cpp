...
// The single source of truth for a player's dungeon role.
//
// Talent tab only: no shapeshift form, no presence, no active AI strategy.
// Those all describe what someone is doing this second, and a role has to
// survive standing in a city out of form with the AI not yet built.
//
// Before this existed, the same feature answered the question two different
// ways and they disagreed. LfgJoinAction::GetRoles (what a bot QUEUES as) read
// the talent tab; LfgFillRoleOf (what the group fill COUNTS, and what the party
// frame role icon gets) read the bot's active AI strategy. So a bot could queue
// as a healer and be counted as dps, or the reverse -- healers marked as dps,
// dps marked as healers, and a group with no real healer wiping.
//
// Mirrors AiFactory::AddDefaultCombatStrategies exactly for the one genuinely
// ambiguous case, feral druid: that function picks cat when the druid has Cat
// Form and no Thick Hide and bear otherwise, so this has to make the same call
// or the role would again disagree with the behaviour.
uint8 PlayerbotAI::GetDungeonRole(Player* player)
{
    // Same two ids AiFactory keys the feral cat/bear decision off. Redeclared
    // rather than shared because they are file-scope constants over there.
...
bool PlayerbotAI::IsTank(Player* player, bool bySpec)
{
    ...
    {
        case CLASS_DEATH_KNIGHT:
            // Blood only. This used to accept ANY death knight standing in
            // Frost Presence, which is most frost and unholy dps for most of a
            // fight -- and real players have no bot AI, so they always land on
            // this branch. A frost dps in a dungeon group was being counted as
            // that group's tank, and the LFG fill then added no tank at all.
...
bool PlayerbotAI::IsLiveSessionPlayer(Player* player)
{
    if (!player)
        return false;
...
Player* PlayerbotAI::FindLiveGroupMemberForSpeed()
{
    if (!bot)
        return nullptr;

    // BG/arena groups contain the whole faction. Only follow the assigned live master.
    if (bot->InBattleground() || bot->InArena())
    {
        if (master && master != bot && IsLiveSessionPlayer(master) && IsValidPlayer(master))
            return master;
        return nullptr;
    }

    Player* liveLeader = nullptr;
    Player* nearestLive = nullptr;
    ...
    if (Group* group = bot->GetGroup())
    {
        ...
        {
            if (leader != bot && IsLiveSessionPlayer(leader) && IsValidPlayer(leader))
                liveLeader = leader;
        }
        ...
        {
            Player* member = gref->GetSource();
            if (!member || member == bot || !IsLiveSessionPlayer(member) || !IsValidPlayer(member))
                continue;
            ...
            if (!nearestLive || dist < nearestDist)
            {
                nearestLive = member;
                nearestDist = dist;
            }
        ...
        if (liveLeader && liveLeader->GetMapId() == bot->GetMapId())
            return liveLeader;
...
void PlayerbotAI::ApplyFollowSpeedRates(float runRate, float walkRate)
{
    if (!bot)
        return;
    ...
    if (std::fabs(bot->GetSpeedRate(MOVE_RUN) - runRate) > 0.01f)
        bot->SetSpeed(MOVE_RUN, runRate, true);
    if (std::fabs(bot->GetSpeedRate(MOVE_WALK) - walkRate) > 0.01f)
        bot->SetSpeed(MOVE_WALK, walkRate, true);

    followSpeedOverrideActive = true;
}

void PlayerbotAI::RestoreFollowSpeedIfNeeded()
{
    if (!followSpeedOverrideActive || !bot)
        return;

    bot->UpdateSpeed(MOVE_WALK, true);
    bot->UpdateSpeed(MOVE_RUN, true);
    followSpeedOverrideActive = false;
}

void PlayerbotAI::SyncFollowSpeedWithLivePlayer()
{
    if (!sPlayerbotAIConfig.matchRealPlayerFollowSpeed || !bot || IsRealPlayer())
        return;

    uint32 const now = getMSTime();
    if (lastFollowSpeedSyncMs && getMSTimeDiff(lastFollowSpeedSyncMs, now) < sPlayerbotAIConfig.realPlayerFollowSpeedSyncMs)
        return;
    lastFollowSpeedSyncMs = now;

    // Battleground/arena raids include every faction member. Only match speed for bots that
    // actually belong to a live player (party follow / addclass / alt), not random BG fills.
    bool groupedWithLive = HasRealPlayerMaster();
    if (!groupedWithLive && bot->GetGroup() && !bot->InBattleground() && !bot->InArena())
        groupedWithLive = FindLiveGroupMemberForSpeed() != nullptr;

    if (!groupedWithLive)
    {
        RestoreFollowSpeedIfNeeded();
        return;
    }

    Player* live = FindLiveGroupMemberForSpeed();
    if (live && live->GetMapId() == bot->GetMapId())
    {
        float const runRate = live->GetSpeedRate(MOVE_RUN);
        float const walkRate = live->GetSpeedRate(MOVE_WALK);
        if (runRate > 0.01f)
        {
            float mult = sPlayerbotAIConfig.realPlayerFollowSpeedMult;
            if (mult < 0.01f)
                mult = 1.0f;
            float const walk = walkRate > 0.01f ? walkRate : runRate;
            ApplyFollowSpeedRates(runRate * mult, walk * mult);
            return;
        }
    }

    float bonus = sPlayerbotAIConfig.realPlayerFollowSpeedBonus;
    if (bonus < 1.40f)
        bonus = 1.40f;

    bot->UpdateSpeed(MOVE_WALK, false);
    bot->UpdateSpeed(MOVE_RUN, false);
    ApplyFollowSpeedRates(bot->GetSpeedRate(MOVE_RUN) * bonus, bot->GetSpeedRate(MOVE_WALK) * bonus);
}
