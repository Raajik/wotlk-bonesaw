...
public:
    ...
    // Deferred teleport pass for bots filled into an LFG group (report #149):
    // the fill hook runs inside LFGMgr::MakeNewGroup BEFORE it writes the
    // group's dungeon id, and the leader's queued dungeons can hold a RANDOM
    // entry whose LFGDungeonData has no entrance coords -- so the teleport
    // resolves the group's own specific dungeon one update pass later.
    // Driven from the world-tick hook by UpdateLfgFillTeleports, which spaces
    // the attempts 5s apart; it must NOT be driven from UpdateAIInternal,
    // whose pass cadence (RandomBotUpdateInterval scaling) sits at 30-60s and
    // starved every deadline short of a minute (report #165).
    void ProcessLfgFillTeleports();
...
private:
    ...
    // Bots filled into an LFG group awaiting teleport to the group's dungeon
    // (guid, time after which a bot that still cannot be teleported is dropped).
    std::list<std::pair<ObjectGuid, time_t>> lfgFillTeleports;
    // Earliest world tick at which UpdateLfgFillTeleports may attempt the
    // next batch (attempts are spaced 5s apart, report #165).
    time_t lfgFillTeleportRetryAt = 0;
