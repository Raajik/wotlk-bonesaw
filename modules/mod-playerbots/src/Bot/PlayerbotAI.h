...
public:
    ...
    // Match (or restore) bot run/walk speed when grouped with a live player.
    void SyncFollowSpeedWithLivePlayer();
...
private:
    ...
    static bool IsLiveSessionPlayer(Player* player);
    Player* FindLiveGroupMemberForSpeed();
    void RestoreFollowSpeedIfNeeded();
    void ApplyFollowSpeedRates(float runRate, float walkRate);
...
protected:
    ...
    uint32 lastFollowSpeedSyncMs = 0;
    bool followSpeedOverrideActive = false;
};
