...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 /*elapsed*/, bool /*minimal*/)
{
    ...
    // LFG fill teleports are NOT retried here: this pass re-arms at
    ...
    {
        // Tightened from 35s 2026-08-21 (request: fill group content "as
        // fast as possible") -- was the real-player-facing latency floor
        // before EnsureQueueFillBots's own bot-selection logic even runs.
    ...
    {
        // Tightened from 30s 2026-08-21, same reasoning as the BG poll above.
...
void RandomPlayerbotMgr::CheckLfgQueue()
{
    ...
    for (std::vector<Player*>::iterator i = players.begin(); i != players.end(); ++i)
    {
        ...
        if (sScriptMgr->OnPlayerCanSoloQueue(player))
            continue;
...
// The role a player would actually fill in a dungeon.
//
// Delegates to PlayerbotAI::GetDungeonRole, which reads the talent tab and
// nothing else. It used to ask the bot's ACTIVE STRATEGY instead, which is a
// different question with a different answer: a bot queues with the tab-based
// role (LfgJoinAction::GetRoles) and was then counted here by strategy, so the
// two disagreed and groups were assembled around a healer that was really a
// dps. One function answers it for both now.
...
static bool RespecBotForRole(Player* bot, uint8 role)
{
    ...
    for (int specNo = 0; specNo < MAX_SPECNO; ++specNo)
    {
        ...
        {
            LOG_INFO("module.playerbots.lfg", "LFG fill: respecced bot {} to '{}' to cover a wanted role", bot->GetName(), name);
            return true;
        }
    }
...
static bool IsIdleFillBot(Player* bot)
{
    ...
    if (!ai || ai->IsRealPlayer() || ai->HasActivePlayerMaster())
        return false;
...
void RandomPlayerbotMgr::EnsureQueueFillBots()
{
    static time_t lastFill = 0;
    time_t const now = time(nullptr);
    // Tightened from 20s 2026-08-21, same reasoning as the BG/LFG polls above.
    if (lastFill && now < lastFill + 8)
        return;
    lastFill = now;
    ...
    if (sPlayerbotAIConfig.randomBotJoinBG)
    {
        for (auto const& queueTypePair : BattlegroundData)
        {
            BattlegroundQueueTypeId const queueTypeId = BattlegroundQueueTypeId(queueTypePair.first);
            if (BattlegroundMgr::BGArenaType(queueTypeId))
                continue;
            BattlegroundTypeId const bgTypeId = sBattlegroundMgr->BGTemplateId(queueTypeId);
            Battleground* bg = sBattlegroundMgr->GetBattlegroundTemplate(bgTypeId);
            if (!bg)
                continue;
            uint32 const teamSize = bg->GetMaxPlayersPerTeam();
            for (auto const& bracketPair : queueTypePair.second)
            {
                BattlegroundInfo const& info = bracketPair.second;
                if (!info.activeBgQueue)
                    continue;
                if (!info.bgAlliancePlayerCount && !info.bgHordePlayerCount)
                    continue;
                uint32 const want = teamSize * (info.activeBgQueue + info.bgInstanceCount);
                BracketNeed need;
                need.minLevel = info.minLevel ? info.minLevel : 1;
                need.maxLevel = info.maxLevel ? info.maxLevel : 80;
                uint32 const allianceHave = info.bgAllianceBotCount + info.bgAlliancePlayerCount;
                uint32 const hordeHave = info.bgHordeBotCount + info.bgHordePlayerCount;
                need.allianceNeed = want > allianceHave ? want - allianceHave : 0;
                need.hordeNeed = want > hordeHave ? want - hordeHave : 0;
                if (need.allianceNeed || need.hordeNeed)
    ...
    {
        auto addLfgNeed = [&](TeamId team, uint32 count)
        {
            if (!count)
                return;
            uint32 minLevel = 80;
            uint32 maxLevel = 1;
            for (uint32 dungeonId : LfgDungeons[team])
            {
                lfg::LFGDungeonData const* dungeon = sLFGMgr->GetLFGDungeon(dungeonId);
                if (!dungeon)
                    continue;
                minLevel = std::min<uint32>(minLevel, dungeon->minlevel);
                maxLevel = std::max<uint32>(maxLevel, dungeon->maxlevel);
            }
            if (maxLevel < minLevel)
                return;
            BracketNeed need;
            need.minLevel = minLevel;
            need.maxLevel = maxLevel;
            if (team == TEAM_ALLIANCE)
                need.allianceNeed = count;
            else
                need.hordeNeed = count;
            needs.push_back(need);
            lfgBrackets.push_back({team, minLevel, maxLevel});
        };
        uint32 const allianceLfg = uint32(LfgDungeons[TEAM_ALLIANCE].size());
        uint32 const hordeLfg = uint32(LfgDungeons[TEAM_HORDE].size());
        auto lfgNeedCount = [&](TeamId team) -> uint32
        {
            uint32 need = 0;
            for (uint32 dungeonId : LfgDungeons[team])
            {
                uint32 const maxPlayers = sLFGMgr->GetDungeonMaxPlayers(dungeonId);
                uint32 const fill = maxPlayers > 1 ? maxPlayers - 1 : 0;
                need = std::max(need, fill);
            }
            return need;
        };
        addLfgNeed(TEAM_ALLIANCE, allianceLfg ? lfgNeedCount(TEAM_ALLIANCE) : 0);
        addLfgNeed(TEAM_HORDE, hordeLfg ? lfgNeedCount(TEAM_HORDE) : 0);
    }
    ...
    uint32 retargeted = 0;
    uint32 const kMaxRetarget = 40;
    uint32 const dkMin = sWorld->getIntConfig(CONFIG_START_HEROIC_PLAYER_LEVEL);

    for (BracketNeed& need : needs)
    {
        if (retargeted >= kMaxRetarget)
            break;

        uint32 inRangeAlliance = 0;
        uint32 inRangeHorde = 0;
        std::vector<Player*> idleAlliance;
        std::vector<Player*> idleHorde;
        for (auto const& pair : playerBots)
        {
            Player* bot = pair.second;
            if (!IsRandomBot(bot) || !IsIdleFillBot(bot))
                continue;
            uint32 const level = bot->GetLevel();
            bool const inRange = level >= need.minLevel && level <= need.maxLevel;
            if (bot->GetTeamId() == TEAM_ALLIANCE)
            {
                if (inRange)
                    ++inRangeAlliance;
                else
                    idleAlliance.push_back(bot);
            }
            else
            {
                if (inRange)
                    ...
                    idleHorde.push_back(bot);
            }
        }

        if (need.allianceNeed > inRangeAlliance)
            need.allianceNeed -= inRangeAlliance;
        else
            need.allianceNeed = 0;
        if (need.hordeNeed > inRangeHorde)
            need.hordeNeed -= inRangeHorde;
        else
            need.hordeNeed = 0;

        auto retargetList = [&](std::vector<Player*>& pool, uint32& remaining)
        {
            while (remaining && !pool.empty() && retargeted < kMaxRetarget)
            {
                Player* bot = pool.back();
                pool.pop_back();
                uint32 level = urand(need.minLevel, need.maxLevel);
                if (bot->getClass() == CLASS_DEATH_KNIGHT && level < dkMin)
                {
                    if (need.maxLevel < dkMin)
                        continue;
                    level = urand(dkMin, need.maxLevel);
                }
                PlayerbotAI* botAI = GET_PLAYERBOT_AI(bot);
                if (!botAI)
                    continue;
                ...
                --remaining;
                ++retargeted;
                LOG_INFO("module.playerbots.lfg", "Retargeted bot {} to level {}-{} for queue fill",
                         bot->GetName(), need.minLevel, need.maxLevel);
            }
        };
...
void RandomPlayerbotMgr::EnsureLfgRoleBots(TeamId team, uint32 minLevel, uint32 maxLevel)
{
    ...
    for (uint8 role : roles)
    {
        ...
        if (!covered && !LoginExtraBotsForRole(team, role, 2))
            LOG_INFO("module.playerbots.lfg", "No bot available to cover the LFG {} shortage for levels {}-{}",
...
uint32 RandomPlayerbotMgr::LoginExtraBotsForRole(TeamId team, uint8 role, uint32 count)
{
    ...
    if (queued)
        LOG_INFO("module.playerbots.lfg", "Logging in {} extra bot(s) over the cap for an LFG {} shortage", queued,
                 role == lfg::PLAYER_ROLE_TANK ? "tank" : "healer");

    return queued;
}

void RandomPlayerbotMgr::FillLfgRaidGroup(Group* group, uint32 maxPlayers, uint8 minLevel, uint8 maxLevel)
{
    if (!group || !sPlayerbotAIConfig.randomBotJoinLfg || maxPlayers <= group->GetMembersCount())
        return;
    ...
    while (group->GetMembersCount() < maxPlayers)
    {
        ...
        for (auto const& pair : playerBots)
        {
            ...
            if (!fallback)
                fallback = candidate;
            // Only a class that could actually play the wanted role is worth a
            // respec -- never retalent a mage into a tank attempt.
            if (!respec && ClassCanFillRole(candidate->getClass(), wanted))
                respec = candidate;
        }

        // Report #149: when nobody idle already plays the wanted role, a class
        // that can play it is respecced instead of silently padding the group
        // with another dps ("1 tank/healer and 4 dps" groups). The respec
        // verifies the resulting talent tab, so a failed template costs a
        // retry on the next add, not a wrong bot.
        if (!bot)
            bot = respec ? respec : fallback;
        if (!bot)
            break;

        if (bot == respec)
            RespecBotForRole(bot, wanted);
        ...
        // Report #165: the retry cadence lives in UpdateLfgFillTeleports, which
        // re-attempts every 5 seconds from the world tick until the bot is on
        // the dungeon's map (the entrance square where MakeNewGroup puts the
        // party -- "in party range"). The deadline only bounds a bot that
        // genuinely cannot go for the whole window (dead and never revived,
        // endless combat); entries otherwise drop on arrival, on logout, or
        // when the group stops being an LFG group, so the list cannot leak.
        lfgFillTeleports.push_back({bot->GetGUID(), NowSeconds() + 600});
    }

    if (added)
        LOG_INFO("module.playerbots.lfg",
                 "LFG fill: added {} bot(s) to a group of {}/{} ({} tank, {} healer, {} dps; wanted {}/{} tank/healer)",
                 added, group->GetMembersCount(), maxPlayers, addedTanks, addedHealers, addedDps, tanksWanted,
                 healersWanted);
}

void RandomPlayerbotMgr::UpdateLfgFillTeleports()
{
    if (lfgFillTeleports.empty())
        return;

    // Called from the world-tick hook (PlayerbotsScript::OnUpdate) every
    // update; the actual work is spaced 5s apart, exactly as report #165
    // asks. First attempt happens on the first tick after the fill.
    time_t const now = NowSeconds();
    if (now < lfgFillTeleportRetryAt)
        return;

    lfgFillTeleportRetryAt = now + 5;
    ProcessLfgFillTeleports();
}
...
void RandomPlayerbotMgr::ProcessLfgFillTeleports()
{
    if (lfgFillTeleports.empty())
        return;

    std::list<std::pair<ObjectGuid, time_t>> remaining;
    for (auto const& [guid, deadline] : lfgFillTeleports)
    {
        ...
        {
            LOG_WARN("module.playerbots.lfg", "LFG fill teleport expired: bot {} never reached the dungeon", bot->GetName());
            continue;
        }

        lfg::LFGDungeonData const* dungeon = sLFGMgr->GetLFGDungeon(sLFGMgr->GetDungeon(group->GetGUID()));
        if (!dungeon || (dungeon->map == 0 && dungeon->x == 0.0f && dungeon->y == 0.0f && dungeon->z == 0.0f))
        {
            remaining.push_back({guid, deadline});
            continue;
        }

        if (bot->GetMapId() == dungeon->map)
            continue; // already there -- MakeNewGroup teleported this member itself

        // Same refusals LFGMgr::TeleportPlayer raises, retried on the next pass
        // instead of dropped with a client-side error the bot never sees.
        if (!bot->IsAlive() || bot->IsInCombat() || bot->IsFalling() || bot->IsBeingTeleported() ||
            bot->GetCharm() || bot->IsMirrorTimerActive(FATIGUE_TIMER))
        {
            remaining.push_back({guid, deadline});
            continue;
        }

        if (bot->TeleportTo(dungeon->map, dungeon->x, dungeon->y, dungeon->z, dungeon->o))
            LOG_INFO("module.playerbots.lfg", "LFG fill: teleported bot {} to dungeon {}", bot->GetName(), dungeon->name);
        else
            remaining.push_back({guid, deadline});
    }

    lfgFillTeleports = std::move(remaining);
}
...
void RandomPlayerbotMgr::CheckPlayers()
{
    ...
    uint32 highest = sPlayerbotAIConfig.randomBotMinLevel;
    for (std::vector<Player*>::iterator i = players.begin(); i != players.end(); ++i)
    {
        Player* player = *i;
        if (!player)
            continue;
        if (player->GetLevel() > highest)
            highest = player->GetLevel();
    }

    uint32 const worldMax = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);
    playersLevel = std::min(highest + 3, worldMax);
    LOG_INFO("playerbots", "Max player level is {}, max bot level set to {}", highest, playersLevel);
}
...
void RandomPlayerbotMgr::Randomize(Player* bot)
{
    ...
    {
        uint32 syncMax = std::max(sPlayerbotAIConfig.randomBotMinLevel,
            std::min(playersLevel ? playersLevel : sPlayerbotAIConfig.randombotStartingLevel,
                sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL)));
        if (bot->GetLevel() > syncMax)
        {
            RandomizeFirst(bot);
            return;
        }
...
void RandomPlayerbotMgr::OnPlayerLogin(Player* player)
{
    ...
    else
    {
        ...
        if (sPlayerbotAIConfig.syncLevelWithPlayers)
        {
            uint32 const cap = std::min<uint32>(player->GetLevel() + 3, sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL));
            if (cap > playersLevel)
                playersLevel = cap;
        }
