...
{
    // Report #118 (recurring): this action is the "give a nearby player
    // something useful" behaviour, and for a bot nobody invited its target is
    // a real player. TradeAction's initiate gate is the enforcement point,
    // but stopping the behaviour at the source too keeps the trigger from
    // selecting the action and burning its cooldown on a refusal every tick.
