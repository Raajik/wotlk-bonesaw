...
bool TradeAction::Execute(Event event)
{
    ...
    if (!bot->GetTrader())
    {
        ...
        //
        // Report #118 (recurring): the first cut gated only IsRandomBot, which
        // left the addclass bot family free to open trades the same way. Both
        // bot kinds refuse; a bot with a real-player master is untouched.
        if ((sRandomPlayerbotMgr.IsRandomBot(bot) || sRandomPlayerbotMgr.IsAddclassBot(bot)) && !botAI->HasRealPlayerMaster())
            return false;
