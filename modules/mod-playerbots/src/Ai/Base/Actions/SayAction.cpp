...
void ChatReplyAction::ChatReplyDo(Player* bot, uint32& type, uint32& guid1, std::string& msg, std::string& chanName, std::string& name)
{
    ...
    // item links posted by players or bots -- react to the drop instead of the generic reply
    if (msg.find("|Hitem") != std::string::npos && HandleItemLinkReply(bot, chatChannelSource, name))
    {
        return;
    }
...
bool ChatReplyAction::HandleItemLinkReply(Player* bot, ChatChannelSource chatChannelSource, std::string const& name)
{
    ...
    placeholders["%random_inventory_item_link"] =
        botItems.size() > 0
            ? GET_PLAYERBOT_AI(bot)->GetChatHelper()->FormatItem(botItems[urand(0, botItems.size() - 1)]->GetTemplate())
            : PlayerbotTextMgr::instance().GetBotText("string_empty_link");

    std::string responseMessage = PlayerbotTextMgr::instance().GetBotText("reply_item_link", placeholders);
