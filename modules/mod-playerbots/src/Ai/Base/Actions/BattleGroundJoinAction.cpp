...
bool BGJoinAction::isUseful()
{
    ...
    // do not try right after login unless a real player is waiting in this bot's bracket
    bool humanWaiting = false;
    uint8 const botLevel = bot->GetLevel();
    for (auto const& queueTypePair : sRandomPlayerbotMgr.BattlegroundData)
    {
        for (auto const& bracketPair : queueTypePair.second)
        {
            ...
            if (!info.bgAlliancePlayerCount && !info.bgHordePlayerCount)
                continue;
            if (botLevel >= info.minLevel && botLevel <= info.maxLevel)
            {
                humanWaiting = true;
                break;
            }
    ...
    if (!humanWaiting && (time(nullptr) - bot->GetInGameTime()) < 120)
        return false;
