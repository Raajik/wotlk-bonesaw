...
public:
    ...
    void OnUpdate(uint32 diff) override
    {
        ...
        sRandomPlayerbotMgr.UpdateLfgFillTeleports();  // Report #165: retry LFG fill teleports every 5s, independent of the manager pass
    }
};
...
public:
    ...
    bool OnLfgAllowBotFill(lfg::Lfg5Guids const& /*check*/, uint32 waitedSec, bool isRaid,
        bool hasSoloQueuePlayer) override
    {
        if (!sPlayerbotAIConfig.randomBotJoinLfg || isRaid || hasSoloQueuePlayer)
            return false;
        return waitedSec >= sPlayerbotAIConfig.lfgDungeonBotFillSec;
    }

    void OnPlayerbotFillLfgDungeon(Group* group, uint32 maxPlayers, uint8 minLevel, uint8 maxLevel) override
    {
        sRandomPlayerbotMgr.FillLfgRaidGroup(group, maxPlayers, minLevel, maxLevel);
    }

    void OnPlayerbotFillLfgRaid(Group* group, uint32 maxPlayers, uint8 minLevel, uint8 maxLevel) override
    {
        sRandomPlayerbotMgr.FillLfgRaidGroup(group, maxPlayers, minLevel, maxLevel);
    }
